...
describe('hosted environment detection', () => {
	it('treats production and the app.s.lan hosted dev domain as hosted', () => {
		...
		assert.equal(isHostedAppUrl('http://app.s.mac.lan'), true);
		assert.equal(isHostedHostname('app.s.mac.lan'), true);
	});
	...
	it('treats bare s.lan / mcp.s.lan and generic local URLs as NOT hosted (plain dev)', () => {
		...
		assert.equal(isHostedAppUrl('http://s.mac.lan'), false);
		assert.equal(isHostedHostname('mcp.s.mac.lan'), false);
