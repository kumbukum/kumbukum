...
Object.assign(swaggerSpec.paths, {
	...
	'/obsidian/connections/{connectionId}/manifest': { post: { tags: ['Obsidian Sync'], summary: 'Upload or reconcile a batched scoped-vault manifest', description: 'Send up to 500 files per numbered batch with complete=false, then finalize the manifest_id with batch_count and complete=true. Preview finalization is non-mutating. Omit scope for legacy full-vault behavior.', security: [{ ObsidianOAuth: ['vault:read', 'vault:write'] }], parameters: [{ name: 'connectionId', in: 'path', required: true, schema: { type: 'string' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { manifest_id: { type: 'string' }, batch_index: { type: 'integer', minimum: 0, maximum: 199 }, batch_count: { type: 'integer', minimum: 0, maximum: 200 }, complete: { type: 'boolean' }, preview: { type: 'boolean' }, summary_only: { type: 'boolean', description: 'Return only aggregate action counts and bytes during preview.' }, scope: { $ref: '#/components/schemas/ObsidianSyncScope' }, files: { type: 'array', maxItems: 500, items: { $ref: '#/components/schemas/ObsidianManifestEntry' } }, device_id: { type: 'string' }, device_name: { type: 'string' }, platform: { type: 'string' } } } } } }, responses: { ...obsidianErrors, 200: { description: 'Batch acknowledgement or final scoped actions and aggregate summary' } } } },
